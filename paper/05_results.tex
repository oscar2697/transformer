...
\textit{La Figura~\ref{fig:attn-per-head} examina la cabeza 1 (índice base cero) del último nivel sobre las oraciones de prueba de seis tokens de contenido seleccionadas mediante el filtro \texttt{-{}-min-len}. En el codificador, la cabeza 1 presenta una estructura aproximadamente diagonal con un sesgo moderado hacia el primer token —el patrón de \textit{attention sink} descrito por Xiao et al. (2024). En el decodificador, la auto-atención preserva estrictamente la estructura triangular inferior impuesta por la máscara causal, verificación adicional de la corrección de la implementación. La atención cruzada, regenerada con el checkpoint Pre-LN + BPE (BLEU 44.00, val\_ppl 9.29, Tabla~\ref{tab:results}), muestra ahora picos nítidos sobre los tokens fuente relevantes —por ejemplo, en S2 (\textit{I ca n't draw .} $\rightarrow$ \textit{Ich kann nicht zeichnen .}) la masa se concentra sobre \textit{draw} al generar \textit{zeichnen}— en contraste con la distribución difusa del diagnóstico word-level (val\_ppl 149.84). La brecha de perplexity se invierte (9.29 frente a 12.39 del baseline), confirmando que la estabilización Pre-LN resuelve el sub-entrenamiento. La cabeza 1 exhibe ya indicios de especialización, aunque la caracterización completa de las 8 cabezas queda para la versión extendida.}e en Colab.}
